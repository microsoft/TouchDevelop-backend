...
export async function initAsync(): Promise<void> {
    ...
    core.addRoute("POST", "scripts", "", async(req: core.ApiRequest) => {
        ...
        let rawSrc = orEmpty(req.body["raw"]);
        if (req.status == 200 && rawSrc != "") {
            core.checkPermission(req, "post-raw");
        }
        let forceid = orEmpty(req.body["forceid"]);
        if (req.status == 200 && forceid != "") {
        ...
        if (req.status == 200) {
            ...
            let isFork = false;
            let entry3 = await core.getPubAsync(orEmpty(req.body["baseid"]), "script");
            if (entry3 != null) {
                scr.baseid = entry3["id"];
                scr.rootid = entry3["pub"]["rootid"];
                isFork = entry3["pub"]["userid"] != req.userid;
            }
            scr.userid = req.userid;
            scr.mergeids = (<string[]>[]);
            if (req.body.hasOwnProperty("mergeids")) {
                scr.mergeids = td.toStringArray(req.body["mergeids"]);
            }
            scr.name = withDefault(req.body["name"], "unnamed");
            scr.description = orEmpty(req.body["description"]);
            scr.iconbackground = withDefault(req.body["iconbackground"], "#FF7518");
            scr.islibrary = orFalse(req.body["islibrary"]);
            scr.ishidden = orFalse(req.body["ishidden"]);
            scr.userplatform = core.getUserPlatforms(req);
            scr.capabilities = (<string[]>[]);
            scr.flows = (<string[]>[]);
            scr.editor = orEmpty(req.body["editor"]);
            scr.target = orEmpty(req.body["target"]);
            scr.meta = req.body["meta"];
            if (typeof scr.meta != "object" || Array.isArray(scr.meta))
                scr.meta = {};
            scr.iconArtId = orEmpty(req.body["iconArtId"]);
            scr.splashArtId = orEmpty(req.body["splashArtId"]);
            scr.raw = rawSrc;
            scr.unmoderated = !core.callerHasPermission(req, "adult");
